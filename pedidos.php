...
<body>
    ...
    <div class="container-fluid row">
        <form class="col-4 p-3" method="POST">
            ...
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Seleccione Rut</label>
                <select class="form-select" name="id_clientes">
                <?php
                // Consulta para obtener los roles disponibles
                $sql_roles = $conexion->query("SELECT * FROM cliente");
                    while ($rol = $sql_roles->fetch_object()) {
                        echo "<option value='$rol->id'>$rol->rut</option>";
                    }
                ?>
                </select>
            </div>
            ...
            <div class="col-8 p-4">
                <table class="table table-hover table-bordered">
                    ...
                    <tbody>
                        <?php
                        include "modelo/conexion.php";
                        $sql = $conexion->query("SELECT pedidos.id, cliente.rut, cliente.nombre, cliente.apellido, pedidos.dia_creacion, pedidos.kg_hallulla, pedidos.kg_marraqueta FROM pedidos INNER JOIN cliente ON pedidos.id_clientes = cliente.id");
                        while ($datos = $sql->fetch_object()) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="container">
                                        <div>
                                            <a href="modificar_pedido.php?id=<?= $datos->id ?>" class="btn btn-small btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <a onclick="return eliminar()" href="pedidos.php?id=<?= $datos->id ?>" class="btn btn-small btn-danger"> <i class="fa-solid fa-trash-can"></i></a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
